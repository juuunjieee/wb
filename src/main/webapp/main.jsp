<%@page import="com.ulwxbase.domain.cus.UserInfo"%>
<%@page import="com.ulwxbase.domain.db.sys.SysRight"%>
...
<html>
<head>
<% UserInfo info = (UserInfo)request.getSession().getAttribute(CbConstants.SessionKey.USER); %>
...
.accordion-expand {
	background: rgba(0, 0, 0, 0) url("images/ulwxbase.sys/arrow_all.png")
		no-repeat scroll -48px 0;
}
...
<script>
	...
	var menuItemLevel0={
	<%
	List<SysRight> rightList=(List<SysRight>)session.getAttribute("rights");
	String aStr="";
	for(int i=0; i<rightList.size(); i++){
		SysRight sr=rightList.get(i);
		if(sr.getSysRightCode().endsWith("000")){
			aStr=aStr+"'"+sr.getSysRightName()+"':'"+sr.getIcon()+"',";
...
<body id="mybody" class="easyui-layout" style="visibility: hidden;">
	<div id="mynorth" region="north"
		...
		<div id="myheader" style="height: 100%; background: #315B95;">
			<a href="./main.jsp">
				<img src="images/ulwxbase.sys/index_logo_text.png?v=20180120" border="0" style="position: absolute; top: 5px" />
			</a> 
			<span class="myspan">
				<span style="color: white">当前登录用户:<%=info.getUser().getAccount() %>&nbsp;&nbsp;用户ID:<%=info.getUser().getSysUserSno() %></span>&nbsp;
				...
				<a href="javascript:void(0)" id="ModifyPassword" onclick="modifyPassView();" style="color: white">修改密码</a> &nbsp;
				<span style="color: white">|</span>&nbsp; 
	...
	<div region="west" split="true" title=" "
		...
		<div id="leftmenu" class="easyui-accordion"
			...
			<%
				List<SysRight> list = (List<SysRight>) session.getAttribute("rights");
				int count = 0; 
				for (int i = 0; i < list.size(); i++) {

					SysRight right = list.get(i);
					String rightCode = right.getSysRightCode();
					String rightName = right.getSysRightName().trim();
					...
					if (secondLevelThree.equals("000")) {
						...
						for (n = i + 1; n < list.size(); n++) {
							SysRight chRight = list.get(n);
							String chRightCode = chRight.getSysRightCode();
							String chRightName = chRight.getSysRightName().trim();
							String chFirstLevelTwo = chRightCode.substring(0, 2);
							String chSecondLevelThree = chRightCode.substring(2, 5);

							String chUrl = chRight.getSysRightUrl();
			...
			<a id="<%=chRightCode%>" name="addTab_<%=count%>"
				...
				border="0" align="bottom" />&nbsp;<%=chRightName.trim()%></a>
	...
	<div region="south" border="true"
		style="height: 23px; background: #E2EEFE; font-size: 11px; line-height: 23px; text-align: center; overflow: hidden">简易贷@2018</div>
